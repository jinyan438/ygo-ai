...
export async function executeYgoPro2Action(runner, input = {}, formatCurrentState) {
  ...
  return {
    ...
    data: {
      ...
      opponentControlledBy: runner.humanOpponent ? 'human' : 'windbot',
...
class YgoPro2DuelRunner {
  constructor(options) {
    ...
    this.humanOpponent = options.humanOpponent === true || options.opponentAiProfile === 'none';
  ...
  async start() {
    ...
    const firstUpdate = this.humanOpponent ? null : this.waitForUpdate(this.startupTimeoutMs);
    ...
    if (!this.humanOpponent) this.spawnProcess(
      'opponent-windbot',
      this.installation.paths.windbotExecutable,
      opponentArgs,
      dirname(this.installation.paths.windbotExecutable),
    );
    if (firstUpdate) await firstUpdate;
    if (!this.humanOpponent && !this.turnOrderSelectionApplied) {
  ...
  handlePolicyRequest(request) {
    try { appendFileSync('C:\\Users\\inori\\goose\\out\\duel-policy.log', `[${new Date().toISOString()}] type=${request?.type ?? '?'} msg=${request?.message ?? ''} result=${request?.result ?? ''} ep=${request?.episodeId ?? '?'}\n`); } catch (logError) { /* ignore */ }
  ...
  async submitResponse(response, action) {
    ...
    try { appendFileSync('C:\\Users\\inori\\goose\\out\\duel-responses.log', `[${new Date().toISOString()}] len=${responseBytes.length} hex=${Buffer.from(responseBytes).toString('hex')} kind=${action?.kind ?? ''} label=${String(action?.label ?? '').slice(0, 40)}\n`); } catch (logError) { /* ignore */ }
  ...
  failPending(error) {
    try {
      ...
      appendFileSync('C:\\Users\\inori\\goose\\out\\duel-errors.log', `[${new Date().toISOString()}] episode=${this.episodeId}\n${message}\n---\n`);
    } catch (logError) { /* ignore */ }
  ...
  getBridgeStatus() {
    ...
    return {
      ...
      humanOpponent: this.humanOpponent,
...
function validateLaunchOptions(runner) {
  ...
  if (runner.humanOpponent) return;
